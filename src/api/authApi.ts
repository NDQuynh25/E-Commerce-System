...
export const callLoginGoogle = (idToken: string) => {
    return axios.post<IBackendRes<IAccount>>('/api/v1/auth/login/google', { token: idToken })
}
export const callLoginFacebook = (accessToken: string) => {
    return axios.post<IBackendRes<IAccount>>('/api/v1/auth/login/facebook', { token: accessToken })
}
